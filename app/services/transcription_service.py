...
def process_transcription_from_url(url, language):
    ...
    video_path = os.path.join(script_dir, "app", video_filename)
    
    # Download the video
    download_video(url, video_path)
    
    base_name = os.path.splitext(video_filename)[0]
    output_subdir = os.path.join(script_dir, "app", "files", f"{base_name}_{unique_id}")
    tmp_subdir = os.path.join(script_dir, "app", "tmp", f"{base_name}_{unique_id}")
    os.makedirs(output_subdir, exist_ok=True)
    os.makedirs(tmp_subdir, exist_ok=True)
